...
import { Skeleton, Select, Switch, Card, Avatar, Form, Input, Modal, AutoComplete } from 'antd';
  ...
  return (
    <div style={container}>
      <div>
        ...
        <div style={row}><h2>Thành Viên</h2><div></div>
      ...
      <div style={{ display: 'flex', flexWrap: "wrap"}}>
        {data.map(item => {
          return <Card style={{ width: 350,  position: 'relative',margin:'16px 20px'  }} >
            ...
            />
            <p className="ability" onClick={() => {
              console.log(item.emp_id)
              setemp_id(item.emp_id);
              setvisible(true)
            }} style={{ position: 'absolute', top: 20, right: 20, color: 'blue', cursor: "pointer" }}>Đánh giá</p>
            <div><span style={{ fontSize: 15, fontWeight: 500 }}>Manmonth: </span><span>{item.manmonth}tháng</span></div>
            <div><span style={{ fontSize: 15, fontWeight: 500 }} >Nhận xét: </span><span>{item.ability}</span></div>
            <div><span style={{ fontSize: 15, fontWeight: 500 }} >Đánh giá năng lực: </span><span>{item.productivity || "chưa có đánh giá"}</span></div>
          </Card>
        })}
      </div>
      <Modal
      ...
      >
        <Form
          form={form}
          onFinish={(data) => {
            console.log({ ...data, project_id: dataproject.id, emp_id: emp_id })
            form.resetFields();
            const ability = async () => {
              try {
                apiProject.ability({ ...data, project_id: dataproject.id, emp_id: emp_id })
        ...
        >
          ...
          <Form.Item>
            <Button type="primary" htmlType="submit" onClick={() => { }}>Đánh giá</Button>
          </Form.Item>
        </Form>
      </Modal>
      {/* <Modal
